...
class FileStructureObfuscator {
  ...
  async reorganizeFiles(projectPath, newStructure) {
    ...
    for (const [originalPath, newPath] of newStructure.fileMapping) {
      ...
      try {
        ...
        // 检查是否为文本文件，使用相应的移动方式
        if (this.isTextFileForProcessing(sourceFullPath)) {
          await fs.move(sourceFullPath, targetFullPath);
          console.log(`📝 移动文本文件: ${originalPath} -> ${newPath}`);
        } else {
          console.log(`🖼️ 移动二进制文件: ${originalPath} -> ${newPath}`);
          await this.moveBinaryFileSafely(sourceFullPath, targetFullPath);
        }
  ...
  // 安全移动二进制文件（带完整性验证）
  async moveBinaryFileSafely(sourcePath, targetPath) {
    try {
      // 使用 copy + remove 模式确保二进制文件完整性
      await fs.copy(sourcePath, targetPath, { preserveTimestamps: true });
      ...
      console.log(`✅ 二进制文件移动成功: ${path.basename(sourcePath)}`);
    } catch (error) {
      console.error(`❌ 二进制文件移动失败: ${error.message}`);
      throw error;
    }
  }
  ...
  async updatePathReferences(projectPath) {
    try {
      ...
      console.log(`📝 需要处理的文件总数: ${allFiles.length}`);
  ...
  // 检查是否为需要处理引用的文本文件
  isTextFileForProcessing(filePath) {
    const ext = path.extname(filePath).toLowerCase();
    const textExtensions = ['.html', '.htm', '.css', '.js', '.ts', '.jsx', '.tsx'];
    return textExtensions.includes(ext);
  }

  // 检查是否为二进制文件（反向逻辑：非文本文件就是二进制文件）
  isBinaryFile(filePath) {
    return !this.isTextFileForProcessing(filePath);
  }
}
