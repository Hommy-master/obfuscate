...
class HtmlObfuscator {
    ...
    static async obfuscate(html, mapping, options = {}) {
        ...
        if (hexEncode || unicodeEncode || addWhitespace) {
            $('body, head').find('*').contents().each(function () {
                ...
                if (this.type === 'text' &&
                    ...
                    const obfuscatedText = HtmlObfuscator.obfuscateText(originalText, options);
        ...
        // // 打乱元素顺序
        // if (shuffleElements) {
        //     this.shuffleChildElements($('body'), $);
        //     this.shuffleChildElements($('head'), $);
        // }
     ...
     */
    static obfuscateText(text, options) {
        let result = text;
     ...
     */
    static shuffleChildElements($parent, $) {
        // 只处理元素节点
